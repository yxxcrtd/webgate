...
import cn.digitalpublishing.po.SysAccountModuleRelationship;

public class AccountModuleRelationshipDao extends CommonDao<SysAccountModuleRelationship, String> {
	...
	public Integer getCount(Map<String,Object> condition, HqlBean hqlBean)throws Exception{
		...
		try {
			...
			List<SysAccountModuleRelationship> list = hibernateDao.getListByHql(hqlBean.getProperties(), hqlBean.getFields(), hqlBean.getHql()+where,helper.getCondition()==null?null:helper.getCondition().toArray(),hqlBean.getOrder(), hqlBean.getClassName());
