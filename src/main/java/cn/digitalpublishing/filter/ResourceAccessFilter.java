...
public class ResourceAccessFilter implements Filter{
	 ...
	 * 获取资源权限信息
	 */
	private String resourceURL="http://127.0.0.1:8080/webgate/pages/rest/getResources";
	
	/**
	 * 获取功能权限信息
	 */
	private String functionURL="http://127.0.0.1:8080/webgate/pages/rest/getFunctions";
	
	/**
	 * 提示链接
	 */
	private String promptURL="http://127.0.0.1:8080/webgate/common/prompt";
	...
	public void doFilter(ServletRequest servletrequest, ServletResponse servletresponse,FilterChain filterchain) throws IOException, ServletException {
		...
		try{
			...
			if(isPermitted(component,resource,roleId)){//如果被允许访问
				...
				if(ResourceAccessConfig.getInstance().get("resourceFilterParam")==null||"".equals(ResourceAccessConfig.getInstance().get("resourceFilterParam"))||request.getParameter(ResourceAccessConfig.getInstance().get("resourceFilterParam"))==null||ResourceAccessConfig.getInstance().get("resourceFilterType")==null||"".equals(ResourceAccessConfig.getInstance().get("resourceFilterType"))||!ResourceAccessConfig.getInstance().get("resourceFilterType").equals(request.getParameter(ResourceAccessConfig.getInstance().get("resourceFilterParam").toString()))){
					filterchain.doFilter(servletrequest, servletresponse);
					return;
				}
	...
	public void init(FilterConfig filterconfig) throws ServletException {
		...
		ResourceAccessConfig.getInstance().put("resourceURL", this.resourceURL);
		ResourceAccessConfig.getInstance().put("functionURL", this.functionURL);
		ResourceAccessConfig.getInstance().put("promptURL", this.promptURL);
